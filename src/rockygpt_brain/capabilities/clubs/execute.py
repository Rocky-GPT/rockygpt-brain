"""Look up structured student-organization records."""
...
from rockygpt_brain.capabilities.clubs.normalize import matches, query
...
async def run(filters: dict[str, str], now: datetime, data: DataPort) -> list[dict[str, Any]]:
    records = await data.clubs(query(filters, now))
