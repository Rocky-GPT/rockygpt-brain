...
| `OPENAI_CHAT_MODEL` | no (default `gpt-4.1-mini`, `config.py`) | Override to pin a specific model. The default was measured against `gpt-4o-mini` and `gpt-4o` on an 18-question grounding/tool-use comparison: it grounded the most questions (9/11 vs 7 and 8), had the best p90 latency, and was by far the most reliable at calling `submit_answer` instead of answering in prose (1/6 turns needed a re-prompt, against 5/6 for both others). Note that a blank value (`OPENAI_CHAT_MODEL=`) is treated as unset and falls back to this default rather than being sent to the provider. |
